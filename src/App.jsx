  ...
  useEffect(() => {
    if (typeof window.autoLoadAllSources === 'function') {
      window.autoLoadAllSources(
        '/data/succession_dummy_data_600_3.xlsx',
        '/data/dw_lnd_1.csv',
        '/data/completion-detailed-report_1.xlsx'
      );
    }

    // Dual MyLearning slot drag-drop
    ['transcript', 'progress'].forEach(slot => {
      ...
      el.addEventListener('drop', e => {
        ...
        if (e.dataTransfer.files[0]) window._loadDualFile?.(slot, e.dataTransfer.files[0]);
      });
    });
  }, []);
  ...
  return (
    <>
      ...
      <div id="loading-screen" style={{
        ...
        background: 'linear-gradient(145deg,#0A1628 0%,#102A4C 40%,#1B4B8A 100%)',
        ...
        <div style={{
          ...
          <svg viewBox="0 0 24 24" fill="none" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"
            style={{ width: '28px', height: '28px', stroke: 'var(--blt)' }}>
        ...
        <div style={{ textAlign: 'center', zIndex: 1 }}>
          ...
          <div style={{ fontSize: '12px', color: 'var(--t3)', fontWeight: 300 }}>
            Loading dashboard data…
          </div>
        </div>
        <div className="spinner" />
        <div id="loadingError" style={{
          display: 'none', color: '#fca5a5', fontSize: '12px', fontWeight: 500,
          background: 'rgba(239,68,68,.15)', border: '1px solid rgba(239,68,68,.3)',
          borderRadius: '8px', padding: '10px 18px', maxWidth: '380px', textAlign: 'center'
        }} />
      </div>
      ...
      <div id="dashboard" style={{ display: 'none' }}>
        ...
        <div className="page" id="pdfTarget">
          ...
          <div className="filter-bar" id="filterBar">
            <span className="filter-lbl">Filter by:</span>
          </div>
          ...
          <div id="snapInlinePanel" style={{ display: 'none', marginBottom: '1rem' }}>
            <div style={{ background: 'var(--surface)', border: '1.5px solid var(--border2)', borderRadius: 'var(--r)', boxShadow: 'var(--shadowmd)', overflow: 'hidden' }}>
              <div style={{ background: 'linear-gradient(90deg,var(--navy),var(--dark))', padding: '.9rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                ...
                <div onClick={() => window.closeSnapModal?.()} style={{ width: '28px', height: '28px', borderRadius: '6px', background: 'rgba(255,255,255,.1)', border: '1px solid rgba(255,255,255,.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#fff', fontSize: '16px' }}>&#x2715;</div>
              </div>
              <div style={{ padding: '.8rem 1.25rem .4rem', borderBottom: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                  ...
                  <select id="snapFilterSelect" onChange={e => window.setSnapFilter?.(e.target.value)} style={{ padding: '4px 26px 4px 9px', background: 'var(--skyxs)', border: '1px solid var(--border2)', borderRadius: 'var(--rsm)', fontSize: '11px', fontFamily: 'inherit', color: 'var(--navy)', cursor: 'pointer' }}>
            ...
            <div className="clevel-panel">
              ...
              <div className="clevel-body">
                ...
                <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '1px solid var(--border)' }}>
                  ...
                  {[
                    { label: 'Overall — Total C-Position / Total Successor Plans', valId: 'benchStrengthValAll', areaId: 'benchDetailAreaAll' },
                    { label: 'Unique — Total C-Position / Total Successor Plans',   valId: 'benchStrengthVal',    areaId: 'benchDetailArea'    },
                    { label: 'Non-Unique — Total C-Position / Total Successor Plans', valId: 'benchStrengthValNU', areaId: 'benchDetailAreaNU' },
                  ].map(({ label, valId, areaId }) => (
                    <div key={valId} style={{ background: 'var(--skyxs)', border: '1px solid var(--border)', borderRadius: '9px', padding: '.7rem 1rem', marginBottom: '8px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
                        <div className="clevel-tile-label">{label}</div>
                        <div className="clevel-tile-val" id={valId} style={{ flexShrink: 0, whiteSpace: 'nowrap' }}>—</div>
                      </div>
                      <div id={areaId} style={{ marginTop: '8px', paddingTop: '8px', borderTop: '1px solid var(--border)' }}></div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="tab-panel active" id="tab-overview">
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '.8rem', flexWrap: 'wrap' }}>
              <svg viewBox="0 0 24 24" fill="none" stroke="var(--t3)" strokeWidth="2" strokeLinecap="round" style={{ width: '14px', height: '14px', flexShrink: 0 }}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              ...
              <select id="overviewClFilter" style={{ display: 'none' }}><option value="Yes">Yes</option></select>
              <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--t2)', whiteSpace: 'nowrap' }}>Filter:</span>
          ...
          <div className="tab-panel" id="tab-positions">
            ...
            <div className="sim-row" style={{ marginTop: '1rem' }}>
              <div className="card">
                <div className="card-title" style={{ margin: '0 0 .6rem' }}>Positions Without Any Successor (Gaps)</div>
                <table className="btable"><thead id="gapThead"><tr><th>Position</th><th>Level</th><th>C-Level</th><th>Critical</th><th>Risk</th></tr></thead><tbody id="gapBody"></tbody></table>
                <div className="pg-bar" id="pgGapBar"></div>
              </div>
              <div className="card">
                <div className="card-title" style={{ margin: '0 0 .6rem' }}>High-Risk: Critical + No Successor</div>
                <table className="btable"><thead id="hrThead"><tr><th>Position</th><th>Level</th><th>C-Level</th><th>Incumbent</th><th>Risk</th></tr></thead><tbody id="hrBody"></tbody></table>
            ...
            <div className="card mb" style={{ border: '1.5px solid var(--border2)' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '.9rem', flexWrap: 'wrap', gap: '8px' }}>
                <div>
                  ...
                  <p style={{ fontSize: '12px', color: '#5a7fa8', fontWeight: 400, margin: 0 }}>Choose an employee and the position you want them to succeed to. The system will fill the card from data and export a ready-to-use .pptx file.</p>
                </div>
              </div>
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'flex-end', marginBottom: '.85rem' }}>
                <div style={{ flex: 1, minWidth: '200px' }}>
                  ...
                  <select id="scEmp" onChange={() => window.scPreview?.()} style={{ width: '100%', padding: '8px 10px', border: '1px solid var(--border2)', borderRadius: 'var(--rsm)', fontFamily: 'inherit', fontSize: '12px', background: 'var(--skyxs)', color: 'var(--text)' }}>
                ...
                <div style={{ flex: 1, minWidth: '200px' }}>
                  ...
                  <select id="scPos" onChange={() => window.scPreview?.()} style={{ width: '100%', padding: '8px 10px', border: '1px solid var(--border2)', borderRadius: 'var(--rsm)', fontFamily: 'inherit', fontSize: '12px', background: 'var(--skyxs)', color: 'var(--text)' }}>
        ...
        <div className="rd-modal" style={{ maxWidth: '780px' }}>
          <div className="rd-hdr">
            <div>
              ...
              <div className="rd-sub">Upload Learning Transcript and Course Completion files — merged by Employee ID or Email</div>
            </div>
            <div className="rd-close" onClick={() => window.closeMyLearningDualUpload?.()}>&#x2715;</div>
          </div>
          <div className="rd-body" style={{ padding: '24px' }}>
            <div className="dual-grid">
              {['transcript','progress'].map((slot, i) => (
                <div key={slot} className="dual-slot" id={`dualSlot_${slot}`} data-slot={slot}>
                  <div className="dual-slot-num">{i + 1}</div>
                  ...
                  <div className="dual-slot-lbl">{i === 0 ? 'Employee Learning Transcript' : 'Course Progress / Completion'}</div>
                  <div className="dual-slot-desc">{i === 0 ? 'Per-employee transcript records' : 'Completion % and status per course'}</div>
                  <button className="dual-slot-btn" onClick={() => window.pickDualFile?.(slot)}>Choose File</button>
                  <div className="dual-slot-meta" id={`dualMeta_${slot}`}></div>
                </div>
              ))}
            </div>
        ...
        <div className="rd-modal">
          <div className="rd-hdr">
            ...
            <div className="rd-actions">
              <button className="rd-export-btn" onClick={() => window.exportReadinessDetail?.()}>
